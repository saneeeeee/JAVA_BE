...
--		- 괄호안의 숫자가 자리수를 표시하며, 이는 바이트 수가 아닌것을 꼭 기억하기 바랍니다.

-- 2. 실수형 타입
--		- number(6,2)는 소수점을 포함한 총 자리수가 6자리이고 소수점 둘째 자리까지 있는 실수형 값이 설정됩니다
--		- 첫 번째 인자값 6은 소수점 자리수를 포함한 총자리수를 의미하고, 두 번째 인자값 2는 소수점 자리수를 의미합니다
...
--		- char(XX)
--		- char(10) 으로 만들어진 필드에 영문자 5크기 만큼 데이터를 저장하면 실제 크기와 상관없이
...
--		- FLOAT : number 의 하위 타입. 2진수 기준 22바이트
