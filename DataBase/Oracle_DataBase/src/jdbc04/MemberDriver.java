...
public class MemberDriver {
	...
	private static void delete(Scanner sc) {
		...
		int result = mdao.delete( mdto.getMembernum() );
	...
	private static void update(Scanner sc) {
		...
		System.out.printf("Bpoint : %d - 수정할 Bpoint를 입력하세요 -> ", mdto.getBpoint());
		...
		while(true) {
			try {
				...
				System.out.println("날짜를 입력예로 맞춰 다시 입력하세요(입력 예 : 2015-12-31");
				e.printStackTrace();
			}
		}
		if(bd != null) {
			java.sql.Date birth = new java.sql.Date(bd.getTime());
			mdto.setBirth(birth);
		...
		if(jd != null) {
			java.sql.Date joindate = new java.sql.Date(jd.getTime());
