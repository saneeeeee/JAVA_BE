...
--			테이블이란 단어는 실질적으로 데이터를 저장하고 있지 않더라도 계정 사용자는 마치
...
--		- 뷰를 생성핳기 위해서는 실질적으로 데이터를 저장하고 있는 물리적인 테이블이
...
-- 뷰이름으로 조회명령을 저장하고 있다가 뷰이름으로 조회할 때마다 조회 명령이 실행되어
-- 결과를 내놓습니다.

create or replace view rentresult as
select a.rentdate, a.num, b.subject, c.name
