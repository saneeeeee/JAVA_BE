...
//	Object 가 상속한 메서드들 중 toString과 함께 가장 많이 오버라이딩 되는 메서드
public class Extends09_Object03 {
	...
	public static void main(String[] args) {
		...
		// 이미 String 클래스에는 저장된 문자가 리턴되는 toString() 메서드가 오버라이딩
		// 되어 있습니다.

		// == 연산은 변수가 직접적으로 저장한 값으로 비교해주는 연산자라는 것을 감안하면
		// s1 == s2 는 두개의 참조변수값(주소값)이 비교되었다라는것을 알 수 있습니다.
		// 현재의 s1 과 s2 는 같은 주소를 갖고 있다는 말이기도 합니다.

		// String s1 = "Hello"; 실행시에 HEAP 영역에 Hello가 저장되고 그 주소가
		// s1 변수에 저장됩니다.
		// String s2 = "Hello"; 실행시에는 새로운 영역에 Hello가 저장되는게 아니라
		// 이미 존재하는 Hello의 주소를 s2에 또 저장합니다.

		// 이는 모두 new 키워드가 없어서 새로운 공간이 안생겼기 때문에 일어나는 현상입니다.
