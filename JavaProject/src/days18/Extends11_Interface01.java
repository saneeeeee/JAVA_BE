...
//	추상 클래스처럼 일반 멤버변수와 일반 멤버메서드를 가질 수 없습니다
//	인터페이스는 멤버 메서드로 public abstract 메서드만 가질 수 있습니다.
//	다중 상속이 가능합니다
...
interface InterA{
	...
	//	InterA(){}		//	에러. 추상메서드만 생성자 생성이 가능하고, 
							//	인터페이스는 일반메서드와 생성자조차도 생성이 불가능합니다.
	//	public void print() {}		에러	
	//	일반 메서드 선언 불가(추상메서드만 생성 가능)
...
class SubA implements InterA{
	public void test() {
		System.out.println("SubA 클래스의 test 메서드 실행");
	}
}
public class Extends11_Interface01 {
	...
	public static void main(String[] args) {
		...
		InterA a = new SubA();
		//	부모 인터페이스의 레퍼런스를 사용하여 자식 클래스의 오버라이딩된 메서드를 호출
		a.test();
	}
