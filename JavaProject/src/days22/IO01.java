...
public class IO01 {
	...
	public static void main(String[] args) throws IOException {
		...
		//	1바이트(한글자) 읽어서 아스키코드값으로 최종 입력됩니다
		...
		*/
		...
		//	1바이트씩 읽어 올 수 있으며, 읽어 들이는 값은 문자의 코드(아스키 코드)값입니다
		//	따라서 num 변수에는 입력한 글자의 아스키 코드값이 저장됩니다.
		//	A - 65		0 - 48		a - 97
		
		//	new Scanner(System.in) 그동안 사용해오던 Scanner 도 위의 System.in
		//	의 객체를 모체로 하고있습니다. 입력의 시작은 System.in 객체가 됩니다.
		
		//	입력 스트림의 동작 방식
		//	입력 스트림의 경우 데이터를 입력받을 버퍼를 검색하여 해당 버퍼에 내용이 존재하지 않는 경우
		//	버퍼에 데이터가 쌓일 때까지 대기함(키보드 입력의 경우 사용자가 데이터를 입력하고 엔터키를 입력할 때까지)
		//	연결된 버퍼에 데이터가 존재하는 경우 , 해당 버퍼에서
		//	바이트 스트림은 1byte 씩 데이터를 읽어오고, 문자스트림의 경우 2byte 씩 데이터를 읽어옵니다
		//	기본 입력은 1바이트 읽기 실행
		int input;
		System.in.read();
		System.in.read();
