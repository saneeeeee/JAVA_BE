...
<script type="text/javascript">
	function idCheck(){
		if(document.frm.userid.value.length == 0) {
			alert("중복 조회할 아이디를 먼저 입력하고 버 튼을 눌러주세요.")
			document.frm.userid.focus();
			return;
		}
		
		var inputid = document.frm.userid.value;
		var opt = "toolbar=no, menebar=no, scrollbars=yes, resizable=no, width=500, height=200";
		window.open("idcheck.do?userid=" + inputid , "idcheck" , opt);
		//	팝업창 오픈 메뉴의 idcheck.do. 는 서블릿이며, 서블릿에서 아이디 중복체크 작을 한 후 , 결과를 싣고 포워딩되는
		//	페이지에 팝업창에 표시될 예정입니다.
		//	서블릿 호출 시 파라미터를 위와 같이 호출되는 주소에 ? 함께 전달할 수 있습니다.
	}
	...
	function joinCheck(){
		...
		if(document.frm.pwd.value != dacument.frm.pwd_check.value){
