...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 이미 존재하기 때문에 session 을  바로 사용하는것이 가능하지만, 
		// 서블릿은  그렇지 않고, request와   response 를 전달받아 사용하기 때문에 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 전단된 id 로 member  테이블에서 회원을 검색하고, 검색결과에 따라 pwd 와 비교해서
		// 정상 로그인 여부를 결정합니다
		
		// 로그인이 실패했을때를 대비해서 포워딩 할 경로를 먼저 설정합니다
		String url = "member/loginForm.jsp";  
		// 정상로그인시에   url 값이 main.jsp 로 변경됩니다
		
		// Dao 의 메서드를 호출하기 위해서 객체를 생성합니다
		MemberDao  mdao = MemberDao.getInstance();
		//  아이디로 검색해서 해당 아이디의 멤버정보를 모두 dto  형태로 리턴 받습니다.
		...
		if( mdto == null ) {
			...
			request.setAttribute("message", "무슨이유에선지 로그인이 안됩니다.");
		}
