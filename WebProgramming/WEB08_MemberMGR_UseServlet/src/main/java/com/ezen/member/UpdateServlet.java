...
public class UpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		//	수정할 회원을 조회해서 dto 리턴받고, 이를 request에 저장하여 이동합니다.
		//	이 자료는 수정할 입력란에 최초 위치할 데이터로 사용됩니다.
		String userid = request.getParameter("userid");
		MemberDao mdao = MemberDao.getInstance();
		MemberDto mdto = mdao.getMember(userid);
		request.setAttribute("updateMember", mdto);
		//	같은 정보를 session에서 loginUser라는 이름으로 저장하고 있으므로,
		//	현재 코드는 주석처리해도 무방합니다
		*/
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result == 1) {
			...
			request.setAttribute("message","회원 정보수정이 실패하였습니다. 관리자에게 문의하세요.");
		}
