...
// Attribute 에 저장되었던 모든 자료는 getAtrribute로 꺼냈을때 Object 형이므로, 원래의 자료형으로 사용하기 위해ㅓ서는
// ( 자료형 ) 같은 형변환이 필요합니다.
%>
id : <%= id %><br>
pwd : <%= pwd %><br>
...
<%
Enumeration<String> names = session.getAttributeNames(); //세션의 저장된 키값들만 얻어서 저장합니다

//다음 데이터가 없을때까지 반복하면서  다음데이터를 키값으로 사용
while( names.hasMoreElements() ){
	// nextElement 를 String  형 변수에 저장
	String name = names.nextElement().toString(); //(String) 형변환을 사용하지 않고, 형변환 매서드를 사용
